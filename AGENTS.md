...
- **An inbound webhook endpoint under `common/`, `ee/`, `posthog/` or `products/` verifies through `posthog/ingress/`, never a hand-rolled `hmac` check in a view.** `[lint: inbound-webhooks-go-through-ingress]` The provider incarnation owns the signature scheme and the delivery parsing, and the product registers its handlers in `products/<product>/backend/webhook_consumers.py`; read [docs/internal/inbound-webhooks.md](docs/internal/inbound-webhooks.md) before you add a provider or a consumer.
